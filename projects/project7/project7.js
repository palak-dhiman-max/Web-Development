...
result.textContent=`Hi ${name.value} ${surname.value} ,Your zodiac Signs is ${zodiacSigns[(month.value)-1]}. 
${compliments[date.value]} ${victimCardCompliments[((name.value.length)*2)%20]} ${recommendations[(date.value*month.value)%30]}
${predictions[(name.value.length+surname.value.length)%20]}`;
